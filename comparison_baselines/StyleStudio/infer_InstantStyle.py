...
def main(args):
    ...
    ip_ckpt = args.ip_ckpt or "../../InstantX/CSGO/csgo_4_32.bin"
    vae_path = args.vae_path or None
    ...
    if args.fuAttn or args.fuSAttn or args.fuIPAttn:
        ...
        num_sample=1
    if num_sample == 2:
        init_latents = torch.randn((1, 4, 128, 128), generator=generator, device=device, dtype=torch.float16)
        init_latents = init_latents.repeat(num_sample, 1, 1, 1)
        assert torch.equal(init_latents[0], init_latents[1]) is True
    else:
        init_latents = torch.randn((num_sample, 4, 128, 128), generator=generator, device=device, dtype=torch.float16)
...
if __name__=='__main__':
    ...
    parser.add_argument("--base_model_path", type=str, default=None,
                        help="Path or repo id for the SDXL base model")
    parser.add_argument("--image_encoder_path", type=str, default=None,
                        help="Directory containing the IP-Adapter CLIP image encoder")
    parser.add_argument("--ip_ckpt", type=str, default=None,
                        help="IP-Adapter checkpoint (.bin or .safetensors)")
    parser.add_argument("--vae_path", type=str, default=None,
                        help="Optional custom VAE path (defaults to pipeline VAE)")
