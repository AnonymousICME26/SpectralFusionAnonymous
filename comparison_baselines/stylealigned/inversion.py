...
def _encode_image(model: StableDiffusionXLPipeline, image: np.ndarray) -> T:
    original_dtype = model.vae.dtype
    model.vae.to(dtype=torch.float32)
    device = model.vae.device
    image_tensor = torch.from_numpy(image).float().div(255.0)
    image_tensor = (image_tensor * 2 - 1).permute(2, 0, 1).unsqueeze(0).to(device)
    latent_dist = model.vae.encode(image_tensor)["latent_dist"]
    latent = latent_dist.mean * model.vae.config.scaling_factor
    model.vae.to(dtype=original_dtype)
    return latent
