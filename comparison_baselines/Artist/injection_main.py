...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path or repo id for the base diffusion model.",
    )
    parser.add_argument(
        "--vae_path",
        type=str,
        default=None,
        help="Optional path or repo id for a custom VAE to load.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    device_override = torch.device(args.device if args.device else "cuda")
    if device_override.type == "cuda" and not torch.cuda.is_available():
        device_override = torch.device("cpu")

    globals()["device"] = device_override

    base_model_path = args.base_model_path or cfg.get(
        "base_model_path", "stabilityai/stable-diffusion-2-1-base"
    )
    vae_path = args.vae_path or cfg.get("vae_path", None)

    # Load a pipeline
    torch_dtype = (
        torch.float16 if device_override.type == "cuda" else torch.float32
    )
